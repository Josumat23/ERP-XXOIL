...
**Conexión agregada el 2026-09-17 — maestros del rubro → venta.** Las especificaciones del producto y las equivalencias con la competencia existían como maestro y no llegaban a la venta: contestar «¿tienen algo equivalente al Delvac 1340?» eran cuatro pantallas. `/comercial/equivalentes` responde con nuestros productos declarados equivalentes, **qué cubren y qué no**, y cada presentación con precio y disponible (stock menos comprometido), con un enlace que abre la cotización con esa línea cargada. La cobertura se **recalcula**, no se lee la guardada: una homologación vencida se ve en el momento de ofrecer. Véase `docs/equivalente-al-cotizar.md`.
...
**Integración Ventas→Compras/Producción verificada:** `calcularOperaciones()` reúne el saldo no facturado de pedidos pendientes/parciales y lo consume contra el pronóstico mediante `max(pronóstico, backlog)`. El MRP incluye pedidos vencidos/sin fecha y presentaciones no pronosticadas, aislados por compañía. La necesidad usa stock físico una sola vez; no vuelve a restar `stockReservado`, porque representaría duplicar el mismo backlog.
...
| `registrarCalidad` | Bloquea envasado si `RECHAZADO`; **escribe `LoteGranel.densidadKgL`** con el valor medido de la característica marcada `esDensidad` | `ControlCalidad.resultado`, `causaId` (catálogo estructurado `CausaCalidad`), `accionCorrectiva`, `ResultadoCaracteristicaCalidad.valorMedido` | Indirecto: la densidad gobierna la conversión kg→litros de todo comprobante del lote | — | `usuarioId`, `fecha` |
...
**Desconexión cerrada el 2026-09-16 — calidad → conversión de unidades.** El plan de inspección hacía medir la densidad y el valor se quedaba en `ResultadoCaracteristicaCalidad`, sin lector. La densidad que convierte kg en litros en cada comprobante (`LoteGranel.densidadKgL`) se tecleaba aparte en `finalizarLote` — es decir, **antes** de que el laboratorio midiera, porque el lote pasa a `PENDIENTE_CALIDAD` y recién ahí se ensaya. Dos fuentes para el mismo hecho y ganaba la provisional, sin que nada las comparara. `CaracteristicaPlanCalidad.esDensidad` marca cuál medición es la densidad y `registrarCalidad` la escribe en el lote; el campo manual desaparece cuando el plan la mide y el servidor lo rechaza si llega igual. Véase `docs/densidad-desde-calidad.md`.
