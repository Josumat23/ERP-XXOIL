...
| Centro de costo — jerarquía / grupo de centros de costo | Sí, con múltiples plantas | `CentroCosto.parentId`, validación de ciclos y agregación por subárbol en `/finanzas/centros-costo` | **Verificado completo:** árbol multinivel por compañía, reorganización controlada y presupuesto vs. real agregado. | Bajo | Extensión propia siguiendo el patrón auto-relacional de `EdtProyecto` | **P1 completado** |
...
| Acuerdos marco / contratos de suministro | Con proveedores estratégicos de materia prima (aceites base, aditivos) es común a esta escala | Acuerdos vigentes, liberación parcial y determinación automática de fuente desde MRP | **Cubierto** | — | Extensión propia | Entregado 2026-09-09 |
...
| Origen de demanda: pronóstico + pedidos firmes | Sí | `calcularOperaciones()` consulta pedidos pendientes/parciales y devuelve el neteo visible en MRP y Proyecciones | **Verificado completo:** `max(pronóstico, backlog)` consume el pronóstico sin duplicar pedidos; incluye saldos parciales, vencidos/sin fecha y productos no pronosticados. | Bajo | Neteo por consumo y aislamiento por compañía | **P1 completado** |
| Protección de stock comprometido en MRP | Sí | La revisión encontró que `stockReservado` ya había sido incorporado previamente. Al integrar backlog explícito se reemplazó la resta indirecta por la demanda firme equivalente, evitando contar el mismo pedido dos veces. | **Verificado completo y reconciliado:** pedido firme aparece una vez en demanda; necesidad = demanda neteada + mínimo − stock físico. | Bajo | Pruebas de consumo y exceso de backlog | **P1 completado** |
...
| Requisito | Aplicabilidad | Evidencia | Estado | Riesgo | Solución | Prioridad |
|---|---|---|---|---|---|---|
| WBS/EDT jerárquico, red de actividades, ruta crítica (CPM) | Sí | Ver Blueprint 02 L1.7 | **Verificado completo**, con exclusiones deliberadas documentadas (RN-PRY, `000-Governance/010-AI/proyectos/RN.md`) | Bajo | — | — |
...
| Estructura organizativa jerárquica (organigrama, posiciones, jefe→reporte) | Sí, con más empleados y varias plantas esto deja de ser opcional | Jefe→reporte y organigrama por compañía construidos; `cargo`/`área` siguen como texto libre | **Parcial.** Ya existe jerarquía acíclica navegable; faltan posiciones y unidades versionadas y su uso en aprobaciones. | **Alto** | Extensión propia: completar modelo de posición/puesto | **P1 parcial** |
...
| Revisión periódica de accesos | Sí | `000-Governance/010-AI/configuracion-usuarios/RN.md` — alerta de inactividad 90 días; **certificación formal desde el 2026-09-12** (`docs/certificacion-accesos.md`) | ~~**Verificado completo**, alcance simple (alerta, no certificación formal)~~ → **Verificado completo con certificación formal**: campaña que congela lo revisado, decisión por usuario con motivo al revocar, nadie certifica su propio acceso, y revocar desactiva la cuenta de verdad | Bajo | — | — |
| Motor de conflictos de SoD (matriz de combinaciones de permisos riesgosas) | Sí | `src/lib/segregacionFunciones.ts`, bloqueo transaccional en `configuracion/grupos-seguridad/actions.ts` y pruebas `tests/sod-conflicts.test.ts` | **Verificado completo en el alcance P1:** grupos personalizados no pueden combinar operar y aprobar en Materiales, Finanzas, Ventas o RR.HH.; la UI revierte rechazos y diagnostica conflictos existentes. | Bajo | Matriz explícita y testeable, sin reglas inventadas para módulos sin flujo aprobatorio equivalente | **P1 completado** |
...
| Requisito | Aplicabilidad | Evidencia | Estado | Riesgo | Solución | Prioridad |
|---|---|---|---|---|---|---|
| Hub de reportes operativos, comparación de períodos | Sí | `/reportes`, `000-Governance/010-AI/finanzas-comparacion-periodos/RN.md` | **Verificado completo** para el alcance de "reporte operativo sobre datos transaccionales en vivo" | Bajo | — | — |
...
8b. ~~MRP — neteo de reservas y pedidos firmes.~~ **Completado:** backlog explícito consume el pronóstico sin doble conteo.
9. HCM — Estructura organizativa jerárquica + su cruce con capacitación SST.
10. ~~GRC — Motor de conflictos de SoD.~~ **Completado.**
