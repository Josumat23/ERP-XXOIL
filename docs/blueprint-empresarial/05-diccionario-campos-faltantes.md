...
| ~~`plantaId` o equivalente~~ | **No aplica** (negocio, 2026-09-12): 1 planta = 1 almacén. Ver la fila correspondiente en "Campos que siguen faltando" | P1 |
...
| ~~`Almacen` — `plantaId` (separación Werk→Lgort)~~ | P1 | **No aplica, confirmado por el negocio el 2026-09-12: 1 planta = 1 almacén es suficiente.** `Almacen.tipo` ya distingue el rol, y el MRP y la capacidad de Proyecciones ya operan por planta bajo ese supuesto —que deja de ser supuesto—. Modelar una jerarquía Werk→Lgort que nadie usa obligaría a elegir planta **y** almacén en cada movimiento sin aportar control. Si una planta llega a tener bodegas subordinadas, se reabre (Blueprint 10, respuestas registradas). |
...
| ~~`NotaCredito` — `model NotaDebito`~~ | P2 | **Construido el 2026-09-11, acotado al recargo por mora.** El catálogo 10 de SUNAT admite tres tipos; el sistema calcula por su cuenta **uno solo** —el recargo por mora sobre facturas vencidas— y emitir por aumento de valor o penalidad exigiría decidir cuándo corresponde y sobre qué base, decisión del negocio que no se inventa. ~~Los tres códigos están en el enum; solo se emite el 01, y una prueba lo exige.~~ **Catálogo 10 completado el 2026-09-12**, tras confirmar el negocio que también emite por aumento de valor y penalidad. Los tipos 02 y 03 se emiten **a mano**: el sistema aporta el mecanismo y la persona el concepto y el importe, porque cuándo corresponden y sobre qué base sigue siendo criterio del negocio. La afectación al IGV se declara explícitamente, sin opción premarcada — es criterio tributario y no es el mismo para un ajuste de precio que para una penalidad. A diferencia de la de mora, estas **sí** aumentan el saldo y **sí** postean, porque el documento es el hecho económico. Véase `docs/nota-debito-manual.md`. **La nota no vuelve a cargar nada**: el recargo ya subió el saldo y ya posteó su asiento cuando se aplicó, así que emitir el documento no toca saldo ni contabilidad —tres pruebas lo protegen, incluida una que lee el código de la acción—. De paso se corrigió que el adaptador de envío directo a SUNAT elegía el XML con un ternario encadenado: una nota de débito se habría enviado **con la estructura de una guía de remisión** sin que nada lo advirtiera; ahora cada tipo se nombra y la ND se rechaza con un motivo claro en vez de mandar un XML inventado. Véase `docs/nota-debito-mora.md`. |
...
## Campo del rubro que este diccionario nunca registró — construido el 2026-09-16

El diccionario original salió de un fit/gap contra SAP, y por eso no vio lo que es propio de un fabricante de lubricantes y no de una manufactura genérica: **qué normas declara cumplir cada producto**.

| Maestro | Campo faltante | Por qué se necesita | Estado |
|---|---|---|---|
| `Producto` | Especificaciones estructuradas (API, ACEA, JASO, SAE, ISO, NLGI, OEM) y la distinción entre **cumplir** y estar **homologado** | Un distribuidor pregunta «¿cumple API CK-4?» antes de comprar. La respuesta vivía en `notasTecnicas`, texto libre: no se podía filtrar, no respondía «qué productos cubren ACEA E9», y el certificado de análisis no la mencionaba. Peor: aplanaba una declaración propia del fabricante y una aprobación otorgada por un tercero —con número y vigencia verificables— en la misma frase. | **Construido el 2026-09-16.** `EspecificacionTecnica` (catálogo por compañía, **nace vacío**: qué códigos siguen vigentes es conocimiento del rubro que cambia y sembrarlo sería decidir por XXOIL, con una prueba que falla si algún seed lo hace) y `EspecificacionProducto` (qué declara cada producto). Una homologación exige número y vigencia; un «cumple» con número se rechaza en vez de limpiarse en silencio; un código OEM exige su emisor. Se imprimen en el **certificado de análisis**, en bloque aparte y con leyenda, porque no son resultados de ensayo del lote — y una homologación vencida **no se imprime**, porque el documento se emite hoy. Véase `docs/especificaciones-producto.md`. |
| `Producto` | Equivalencias con productos de la competencia | «¿Cuál es tu equivalente al Mobil Delvac 1340?» es media venta en lubricantes y no existía en el sistema. | **Construido el 2026-09-17.** `ProductoCompetencia` (con la **fuente** de lo que se le atribuye), `EspecificacionCompetencia` (lo que su ficha declara, **sin** atribuirle CUMPLE ni HOMOLOGADO, porque no hay forma de saberlo) y `EquivalenciaProducto`. El sistema calcula la **cobertura** —qué especificaciones del competidor cubre nuestro producto— y la equivalencia la declara una persona: que dos lubricantes se reemplacen es criterio técnico, no aritmética de siglas. Con huecos el motivo es obligatorio, pero declararla no se prohíbe. Y hace lo que una tabla de sinónimos no puede: **una homologación nuestra vencida deja de cubrir**, así que la equivalencia se degrada sola y la pantalla muestra «bajó desde N al declararla». Véase `docs/equivalencias-competencia.md`. |

**Balance** (al detectar; el campo nuevo se construyó el mismo día): **16 campos/modelos construidos y verificados**, **2 que dejaron de aplicar** por decisión de negocio, **4 bloqueados** por confirmación profesional o legal (SST, SCTR, SIRE, Ley 29733) y **11 pendientes sin bloqueo**. El campo nuevo detectado en esta revisión (`ConfiguracionEmpresa` por compañía) y el más accionable de los 11 (la **alerta de transacción sin asiento contable**) se construyeron el mismo día; quedaban **9 pendientes sin bloqueo**. **Los nueve quedaron resueltos entre el 2026-09-11 y el 2026-09-12**: siete construidos, `BOLETA` construida tras confirmar el negocio que existe el canal minorista, y `plantaId` cerrado como **no aplica** tras confirmar que 1 planta = 1 almacén es suficiente. Cada fila de arriba dice qué se hizo y con qué límite declarado. Lo único que sigue abierto en este diccionario son los **cuatro ítems bloqueados** por confirmación profesional o legal: SST, SCTR, SIRE y Ley 29733.
