...
| Escalamiento de cobranza (dunning) | ~~`model AvisoCobranza` (1024-1039): 3 niveles (amistoso/formal/final), `diasVencidos` snapshot. Es un **log de avisos emitidos**, sin `estado`/`resuelto` ni motor de reglas de escalamiento automático más allá de la clasificación por días.~~ **Seguimiento cerrado el 2026-09-12**: el aviso registra qué contestó el cliente (`PENDIENTE`, `COMPROMISO_PAGO` con fecha obligatoria, `EN_DISPUTA` con detalle, `SIN_RESPUESTA`), con quién y cuándo lo registró. El **incumplimiento de un compromiso y la cobranza de la factura se derivan**, no se guardan — un estado guardado se queda viejo apenas cambia el saldo o el día, y una prueba estructural falla si alguien les da columna. Véase `docs/seguimiento-cobranza.md`. **Escalamiento cerrado el 2026-09-13**: `PoliticaCobranza` parametriza por compañía los umbrales de cada nivel, los días sin respuesta y la gracia tras un compromiso incumplido, y la pantalla muestra qué corresponde hoy y por qué. La política no se inventa: la fila existe solo si el negocio la definió, y las dos reglas nuevas nacen apagadas. Véase `docs/escalamiento-cobranza.md`. | **Verificado completo** como motor de reglas configurable. El sistema **no emite el aviso ni bloquea al cliente por su cuenta**: en este ERP registrar un aviso deja constancia de un contacto humano, y crearlo por temporizador fabricaría un contacto que no ocurrió. Sin nivel posterior al aviso final (cobranza judicial), que exige definir el procedimiento legal de la empresa. |
...
- **§7 — Escalamiento de cobranza**: ~~`AvisoCobranza` sigue siendo un log de avisos emitidos, sin máquina de estados.~~ — **la mitad no ambigua se cerró el 2026-09-12**: el aviso ya registra la respuesta del cliente y el compromiso de pago, y un compromiso vencido con la factura impaga se marca solo. ~~Falta el escalamiento automático, que es política de cobranza.~~ **Cerrado el 2026-09-13** con `PoliticaCobranza`, que parametriza la política en vez de inventarla y nace apagada. El motor dice qué corresponde y por qué; no emite el aviso, porque en este sistema un aviso registra un contacto humano. De paso destapó que la acción que registra el aviso reimplementaba los umbrales en línea, lo que con umbrales configurables habría grabado un nivel distinto del que la pantalla pedía. Véase `docs/escalamiento-cobranza.md` y `docs/seguimiento-cobranza.md`.
...
| Segmento de crédito | Parcial | **Parcial, con menos brecha**: límite, bloqueo, auditoría del cambio, aprobación del pedido que lo excede, **aprobación del cambio de límite** (el aumento no se aplica hasta resolverse) y **seguimiento del aviso de cobranza** con compromiso de pago e incumplimiento derivado, ambos del 2026-09-12, y **escalamiento automático de cobranza** configurable por compañía (2026-09-13). Pendiente: el nivel posterior al aviso final (cobranza judicial) y el segmento compartido entre compañías |
