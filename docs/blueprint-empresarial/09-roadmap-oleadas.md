...
- **Lo que sigue abierto:** el filtro de aplicación en Configuración. Series de documento, unidades de medida y almacenes ya quedaron acotados a la compañía activa (`docs/aislamiento-multiempresa-series-unidades.md`, `docs/aislamiento-multiempresa-almacenes-calendario.md`). Siguen pendientes **grupos de seguridad** y **usuarios**: usan `requerirRol` en lugar de `requerirRolEmpresaActiva`, crean con el `empresaId` por defecto `"1"` y actualizan por el `id` que llega del navegador sin comprobar la compañía. Hasta cerrarlos no se puede dar por cumplido el criterio de aceptación de este ítem.
