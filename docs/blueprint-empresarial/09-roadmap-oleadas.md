...
- **Lo que sigue abierto:** el filtro de aplicación. Cinco módulos de Configuración —almacenes, unidades de medida, grupos de seguridad, usuarios y series de documento— usan `requerirRol` en lugar de `requerirRolEmpresaActiva`, crean con el `empresaId` por defecto `"1"` y actualizan por el `id` que llega del navegador sin comprobar la compañía. Hasta cerrarlos no se puede dar por cumplido el criterio de aceptación de este ítem.
