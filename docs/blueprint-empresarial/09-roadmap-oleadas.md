...
- **Lo que sigue abierto:** el filtro de aplicación en Configuración. Series de documento y unidades de medida ya quedaron acotadas a la compañía activa (`docs/aislamiento-multiempresa-series-unidades.md`). Siguen pendientes **almacenes**, **grupos de seguridad** y **usuarios**: usan `requerirRol` en lugar de `requerirRolEmpresaActiva`, crean con el `empresaId` por defecto `"1"` y actualizan por el `id` que llega del navegador sin comprobar la compañía. Hasta cerrarlos no se puede dar por cumplido el criterio de aceptación de este ítem.
