...
- **Verificación pendiente de ejecutar por una persona:** el recorrido manual del criterio de aceptación (dos compañías en el navegador, cambiar la activa y confirmar a ojo que no se cruza nada). No se pudo correr aquí porque exigiría apuntar un servidor de desarrollo a `dev.db`, que está protegida. La cobertura automatizada es la suite de aislamiento por consultas, más la auditoría de esquema.
