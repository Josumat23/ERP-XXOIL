...
1. **Nómina básica con aportes legales peruanos** (HCM — ver `HCM.md` y `04-hcm-nomina-investigacion-normativa.md`). **Parcialmente construido y verificado (2026-08-04):** investigación normativa completada (RMV, UIT, EsSalud 9%, ONP 13%, comisiones AFP, tramos de renta de 5ta, fórmulas de gratificación/CTS — todo parametrizado y editable, nunca hardcodeado) y el **cálculo mensual de planilla** construido de punta a punta: `Empleado` con sistema de pensión (ONP/AFP) y asignación familiar, motor de cálculo (`src/lib/planilla.ts`) con retención de 5ta categoría por método simplificado, boleta de pago imprimible, asiento contable automático (`postearPlanilla`), y `/rrhh/planilla/parametros` para mantener RMV/UIT/tasas de AFP versionadas por vigencia. Probado end-to-end con dos empleados (uno ONP, uno AFP Integra): el asiento generado cuadró exacto (debe = haber = S/4,283.70) y los descuentos coincidieron con el cálculo manual. **Pendiente para una siguiente iteración sobre esta misma base:** gratificación de julio/diciembre, CTS, liquidación de desvinculación, y archivo de pago bancario (sin especificación de formato de banco todavía).
