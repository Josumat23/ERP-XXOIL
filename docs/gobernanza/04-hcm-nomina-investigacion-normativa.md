...
2. **Formato del archivo de pago bancario**: ¿qué banco(s) usa XXOil para pagar sueldos? El formato de archivo plano difiere por banco (BCP, BBVA, Interbank, Scotiabank todos tienen especificaciones distintas). Sin esa información no se puede construir el exportador — se puede dejar el neto a pagar visible en pantalla mientras tanto.
