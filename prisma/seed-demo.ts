    ...
    await prisma.$transaction(async (tx) => {
      ...
      const pedido = await tx.pedido.create({
        data: {
          ...
          total,
          tasaIgv: 18,
          igv: Math.round(total * 18) / 100,
          totalConIgv: total + Math.round(total * 18) / 100,
          ...audit,
          detalles: {
            create: venta.lineas.map((l) => ({
              presentacionId: l.presentacion.id,
              cantidad: l.cantidad,
              precioLista: l.presentacion.precio,
              origenPrecio: "BASE",
              descuentoPct: 0,
              descuentoMonto: 0,
      ...
      const factura = await tx.factura.create({
        data: {
          ...
          igvFuncional: igv,
          totalFuncional: totalConIgv,
          saldoFuncional: totalConIgv - montoInicial,
      ...
      if (montoInicial > 0) {
        await tx.cobro.create({
          data: {
            ...
            moneda: "PEN",
            tipoCambio: 1,
            montoFuncional: montoInicial,
            cxcFuncionalAplicada: montoInicial,
            diferenciaCambio: 0,
            medioPago: "TRANSFERENCIA",
            fecha: fechaEmision,
            ...audit,
