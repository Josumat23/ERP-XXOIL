...
export async function crearCotizacion(
  ...
  await prisma.$transaction(async (tx) => {
    if (await tx.cliente.count({ where: { id: clienteId, empresaId, estado: "ACTIVO" } }) !== 1 || await tx.vendedor.count({ where: { id: vendedorId, empresaId, activo: true } }) !== 1) throw new Error("Cliente o vendedor fuera de la empresa activa.");
...
export async function actualizarProbabilidad(
  ...
  if (actualizada.count !== 1) {
    const existe = await prisma.cotizacion.findFirst({ where: { id, empresaId }, select: { id: true } });
    return {
      error: existe
  ...
  try {
    await prisma.$transaction(async (tx) => {
      await tx.cotizacion.updateMany({
        where: { id, empresaId, estado: { in: ["PENDIENTE", "ACEPTADA"] }, pedidoId: null },
        data: { probabilidad: { increment: 0 } },
      });

      const cotizacion = await tx.cotizacion.findFirst({ where: { id, empresaId }, include: { detalles: true } });
      ...
      for (const detalle of [...cotizacion.detalles].sort((a, b) =>
        ...
        const presentacion = await tx.presentacion.update({
          where: { id: detalle.presentacionId, empresaId },
      ...
      const convertida = await tx.cotizacion.updateMany({
        where: { id, empresaId, estado: cotizacion.estado, pedidoId: null },
