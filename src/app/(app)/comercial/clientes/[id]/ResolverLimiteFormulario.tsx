...
import { rechazarCambioLimiteCredito, type EstadoFormulario } from "../actions";

export default function ResolverLimiteFormulario({ solicitudId }: { solicitudId: string }) {
  const accion = rechazarCambioLimiteCredito.bind(null, solicitudId);
  ...
  return (
    <form action={formAction} className="flex flex-wrap items-end gap-2">
      ...
      <button type="submit" disabled={enviando} className="boton-secundario text-sm text-red-700 dark:text-red-400">
        {enviando ? "Rechazando..." : "Rechazar aumento"}
      </button>
