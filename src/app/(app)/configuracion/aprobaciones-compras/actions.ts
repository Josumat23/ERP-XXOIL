...
export async function crearNivelCompra(_estado: EstadoNivel, formData: FormData): Promise<EstadoNivel> {
  ...
  try { await prisma.nivelAprobacionCompra.create({ data: { ...nivel, empresaId: auth.usuario.empresaId } }); }
...
export async function desactivarNivelCompra(id: string): Promise<void> {
  ...
  await prisma.nivelAprobacionCompra.updateMany({ where: { id, empresaId: auth.usuario.empresaId }, data: { activo: false } });
