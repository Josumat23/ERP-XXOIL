...
export async function siguienteNumeroCampanaCertificacion(tx: Tx, empresaId: string): Promise<string> {
  await reservarCorrelativo(tx);
  const ultimo = await tx.campanaCertificacionAccesos.findFirst({
    where: { empresaId },
    orderBy: { numero: "desc" },
  });
  return siguiente("CA", ultimo?.numero ?? null);
}

export async function siguienteCodigoUnidadManipulacion(tx: Tx, empresaId: string): Promise<string> {
  await reservarCorrelativo(tx);
  const ultimo = await tx.unidadManipulacion.findFirst({
    where: { empresaId },
    orderBy: { codigo: "desc" },
  });
  return siguiente("HU", ultimo?.codigo ?? null);
}
